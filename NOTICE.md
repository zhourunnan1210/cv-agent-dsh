...
| paper-fetch | `packages/dsh-plugin/skills/paper-fetch/` | 基准提交 `8e329aa`，2026-09-16 | MIT | https://github.com/Agents365-ai/paper-fetch | DOI / 标题 → PDF（七源回退）。**由 `cv-agent-dsh` 插件包自己拥有并随包分发**（登记在 `package.json` 的 `files`），补上 Asta 没有的全文获取能力。**本仓库对上游做了两处修改**（Sci-Hub 默认关闭），逐条登记在 `packages/dsh-plugin/skills/README.md`。上游 `LICENSE` 原件保留在该目录内。 |
