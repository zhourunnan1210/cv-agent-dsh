...
```
先告诉我库里有多少篇拿到了全文、多少篇已经做过笔记、还差多少篇。
差的那些，一次 3 篇地做，做完一批把结果告诉我，然后接着做下一批，直到做完或我喊停。
每批做完告诉我：这批几篇成功、几篇失败、还剩多少没做。
```

**如果只是一两篇**（比如新到的一篇重要论文），点名就好：

```
先给这一篇做结构化笔记：<论文标题或编号>。
做完告诉我笔记质量、以及它用一句话讲的是什么。
```

**看什么**：每篇有质量标记和一句话摘要；**"还剩多少"这个数会一直跟着报**，据此决定要不要接着做。
**不对劲怎么办**：报"还没有全文"→ 这篇没解析成功，回第 3 步；
报"没按格式回"→ 子代理没按要求输出，**那一篇会单独记失败，不影响同一批的其它篇**，
最后单独重试即可。

> 为什么一次只做 3 篇：每篇都要派一个子代理去读全文，一批太大单次调用会拖很久。
> 一批里某篇失败是正常的——结果里会分开写清哪篇成功、哪篇为什么失败，**不要因为一篇失败就重跑整批**。

> 📌 **开工前先读这一篇**（2026-09-17 精读批次发现的漏网之鱼）：库里有一篇 **ICCV 2025** 的
> `Generalization-preserved learning: Closing the backdoor to catastrophic forgetting in
> continual deepfake detection`，与本课题几乎同题，但**从来没做过笔记**——
> 因此它没进知识库，检索也召不回它。直接点名先做它，再批量补其余的。
