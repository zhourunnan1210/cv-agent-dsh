...
| E33 | §5.2 子代理委派 / §16.1 角色矩阵 | 隐含假设：`maxDepth` 是「子代理还能再往下派几层」（相对语义） | **参数语义理解偏了一层，代价是整条委派链路阻断**。实测 2026-09-17（用户新课题立项，`cvagent_kb_scout`）：`Error: subagent depth 1 exceeds maxDepth 0`。真语义是**子代理自己的绝对层级上限**（`dsh-subagent/lib/index.js:432`：`childDepth = delegationDepthOf(parent) + 1; if (childDepth > maxDepth) throw`）——根会话层级 0 ⇒ 子代理层级 1 ⇒ 写 `0` 时**任何**委派都失败。写死 `0` 的共 **6 处**（检索 / 提取 / 归纳 / idea 生成 / idea 打分 / 写作）**全部**不可用；而 `cvagent_state_*` 等不派子代理的工具照常可用，于是症状看起来像"某一个工具坏了"。官方 `subagent` 工具默认值是 **3**，本来完全跑得通——失败纯由我们传参造成。**测试把错误取值固化成了期望值**：4 个文件都写着 `expect(call.request.maxDepth).toBe(0)` | **L1**（用户真实会话，阻断级） | ① 新增常量 `SUBAGENT_MAX_DEPTH = 3`（与部署默认一致）并替换 6 处；② `src/subagent.ts` 的字段注释改成真语义 + 事故原文；③ `tests/subagent-depth.test.ts` **调真 SDK 的 `resolveChildDepth`**（不抄判据）在根会话/层级 1/层级 2 三种前提下断言可派，并反向断言 `0` 必抛（变异验证：常量改 0 → 报错与生产逐字相同）；④ `tests/subagent-contract.test.ts` 静态扫全部 6 处，禁止字面量 0；⑤ 4 个旧断言 `toBe(0)` → `toBeGreaterThanOrEqual(1)` |
...
> `scripts/calibrate-vector-thresholds.mjs` 在**真实库**上量过。结论是**不能用**：
> 正例（共享论文的条目对）与负例（无共同论文）的余弦分布几乎完全重叠——
> modules 库上负例最大值 0.732 **高于**正例最大值 0.713，任何阈值都分不开；
...
> ⚠️ **原来这里写着"embedding 到位后同一套契约自动升级到 `vector` 模式"——这句已被实测证伪**
> （2026-09-18）。契约确实不用改（`retrieval_mode` / `classifyRisk` / `deriveEvidence`
...
| `cvagent_idea_generate` | N 视角并发 Generator（视角缺省取自问题库）→ 跨视角**合并去重**；每个视角的状态如实区分 `ok / duplicate / empty / error`（"想出来了但别人说过"与"确实没方向"是两种信息）；委派契约：`spawn` + `toolFilter={cvagent_kb_search,cvagent_kb_summary}` + `maxDepth: SUBAGENT_MAX_DEPTH`（**不是 0**，见 E33） |
...
### 12.12 面向用户的表达与流程弹性（2026-09-17 用户反馈，非勘误）

用户以**使用者视角**提了三条，都属于"人和 agent 之间的接口"，不属于机制缺陷——
但按本项目的判据，**要用户一直盯着说明书操作，就是接口没做好**：

| 反馈 | 处理 |
| --- | --- |
| ① 发给 agent 的话里中英术语太多，得对照说明书才能操作 | `docs/Phase3-真实会话操作指引.md` **重写**：每步给一段可以直接念的普通话；术语只在末尾「想自己核对时的对照表」里出现，平时不用看 |
| ② 流程卡得死：本地库已经很丰富时，还要走"检索→下载→解析→提取→归档"未必必要 | 新增 `CorpusMode`（`extend` / `reuse`）：**要不要再找文献由用户裁定**。模型必须先**用白话问**（"就用现有的 / 只补最近两年的 / 重新找一批"），用户答"就用现有的"才落 `cvagent_scope_set(reuse_existing=true)`——门控口径回到绝对口径，存量算数。**模型不能自己跳过检索**（那等于把门控交回给它自己），`reuse` 必须由用户知情选择并落盘，可在 `cvagent_state_get` 里审计 |
| ③ agent 提问要用易懂的中文，别用英文术语 | `instructions.ts` 的常驻约定新增「怎么跟用户说话」四条：平实中文、先结论后口径、选项写成**人能直接判断的取舍**、不把 JSON 原样丢给用户 |

**为什么 `reuse` 是状态而不是开关**：它是"本课题的语料策略"这一事实的一部分，
必须与范围一起落盘、能在 `cvagent_state_get` 里看到、能被门控判据读到；
否则模型每次都可以"这次先跳过"，而不留任何痕迹。

**三条的共同点**：把**判断权**放回用户手里（要不要再检索、够不够用），
同时把**口径**说清楚（存量 / 基线 / 新增各是多少）。这正是本项目对"门控"的一贯要求，
只是这次的对象从"模型"换成了"人"。

### 12.13 P4-2 批量提取：把"提取缺口"变成一等数字（2026-09-17）

精读批次交接（`docs/精读批次交接-2026-09-17.md`）发现一个**系统性缺口**：
库里 **已解析 154 篇，但只有 21 篇做过结构化提取**——四库的 171 条条目基本只覆盖那 21 篇。
更糟的是这个缺口**在工具面上看不见**：`cvagent_kb_summary` 报了"已提取 21"，
却没报"已解析 154"，于是没人能一眼看出积压。

三处改动（都在"让确定的事由代码做，而不是靠模型记"这条线上）：

| 改动 | 为什么 |
| --- | --- |
| `cvagent_kb_summary` 新增 `parsed` 与 `pending_extraction` | 缺口从"要手动查库"变成"总览里的一行数字"。判据早就在用 `parsed`，总览却不给——信息不对称 |
| `PaperLibrary.listUnextracted(limit)` / `countUnextracted()`（新查询） | "哪些还没做"是**确定的查询**；让模型自己报 id 一定会漏、会重、会记错 |
| `cvagent_kb_extract` 支持批量（不传 `paper_id`、传 `limit`） | 154 篇要是逐篇调用，就是 154 次往返；批量把"挑片 + 逐篇处理 + 剩多少"收进一次调用 |

**语义决定**（值得记下来，因为改了既有契约）：

- **单篇失败不再让整次调用 `isError`**，而是落成该篇的 `status: 'failed'` + `error`。
  理由：批量模式下"一篇失败"是正常结果（与 `cvagent_kb_import_papers` 的"部分成功"同一口径）；
  若单篇与批量语义不同，调用方就得按模式分支判断——那是更差的契约。失败信号仍然明确：
  `status` + `error` + 未落库。
- **输出统一为逐篇结果**：`{total, succeeded, failed, results[], remaining}`。
  `remaining` 是"本次之后还剩多少待提取"——**让模型不必自己算就能决定要不要接着调**。
- **`limit` 夹在 1–10**：每篇都是一次子代理委派，一次调用拖太久会撞工具超时。
- **选取顺序 `created_at DESC, paper_id ASC`**：新入库的优先（用户的批次计划就是"本批优先"），
  同一批内按 paper_id 兜底 → **同一状态下每次调用顺序一致**。确定性是能被放心批量跑的前提。

**实测**（真实 `metadata.db`，2026-09-17）：`papers 424 / parsed 154 / extractions 21 /
pending_extraction 133`，`listUnextracted(5)` 给出的正是确定的工作清单。
那篇被点名的 ICCV 2025 撞题论文（`local:zhang …`）也在其中——它是"已解析未提取"，
所以既没进四库、也召不回。
