...
7. **Sci-Hub mirrors** *(**DISABLED by default in this repository** — upstream ships it on. Enable only with `PAPER_FETCH_ALLOW_SCIHUB=1`; see `../README.md` (this plugin's skills register) for the compliance reason)* — last-resort fallback. Tries the mirror list in `PAPER_FETCH_SCIHUB_MIRRORS` (or built-in defaults `sci-hub.ru`, `sci-hub.st`, `sci-hub.su`, `sci-hub.box`, `sci-hub.red`, `sci-hub.al`, `sci-hub.mk`, `sci-hub.ee`) in order; on full miss, scrapes `https://www.sci-hub.pub/` once per process for fresh mirrors. CAPTCHA / missing-paper pages have no PDF iframe and fall through silently.
