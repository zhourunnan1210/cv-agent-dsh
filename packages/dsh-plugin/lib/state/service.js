...
import { createProjectState, requestGate, resolveGate, } from '@cv-research/core';
...
export class ProjectStateService extends Service {
     ...
     */
    async requestStageGate(summary, now = new Date().toISOString()) {
        const state = await this.getOrCreateState();
        const stage = state.current_stage;
        const previous = state.stages[stage] ?? { status: 'in_progress' };
        const staged = {
            ...state,
            stages: {
                ...state.stages,
                [stage]: { ...previous, status: 'in_progress', summary },
            },
        };
        const decision = requestGate(staged, DEFAULT_CRITERIA, now, summary);
        const persisted = decision.gate === null ? staged : { ...staged, pending_gate: decision.gate };
     ...
     * 而不是静默推进。
     */
    async resolveStageGate(decision, comment, now = new Date().toISOString()) {
        const state = await this.getOrCreateState();
