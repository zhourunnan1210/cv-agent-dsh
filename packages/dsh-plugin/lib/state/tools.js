...
export function apply(ctx) {
    ...
    const service = ctx.projectState;
    ...
    toolsRuntime.register(defineTool({
        ...
        description: '请求推进当前阶段（三段式门控第 ① 步）：记录阶段完成摘要并校验完成判据。'
    ...
    toolsRuntime.register(defineTool({
        ...
        description: '设定或切换 ABC 模式并落盘：A confirm（每阶段需用户确认）、B supervised（自动流转可回退）、'
    ...
    toolsRuntime.register(defineTool({
        ...
        description: '落盘门控决议（三段式门控第 ③ 步）：advance 推进到下一阶段并生成回滚快照；'
        ...
        async execute(args) {
            const result = await service.resolveStageGate(String(args.decision), args.comment === undefined ? undefined : String(args.comment));
            return {
