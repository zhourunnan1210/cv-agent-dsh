...
| 新增 group | `cvagent-kb-group`：`isolate: { kb: true }`，内含 `cv-agent-dsh/kb`（论文库服务）、`cv-agent-dsh/kb-tools`（`cvagent_kb_import_paper`）、`cv-agent-dsh/kb-extract`（`cvagent_kb_extract`，Reader 子代理委派提取）与 `cv-agent-dsh/kb-entries`（`cvagent_kb_upsert_entry` / `cvagent_kb_search` / `cvagent_kb_summary`，三库读写面） |
