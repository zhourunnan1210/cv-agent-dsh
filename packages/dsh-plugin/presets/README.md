...
| 新增 group | `cvagent-project-group`：**单一 isolate realm**（`isolate: { projectState: true, kb: true }`），内含 `state` + `state-tools`、`kb` + `kb-tools` + `kb-extract` + `kb-entries` + `kb-research`、`idea-score` + `idea-tools`。2026-09-17 由原先的两个 group 合并：**阶段判据必须读知识库的真实数字**，分 realm 就读不到（详见组合文件里的注释） |
