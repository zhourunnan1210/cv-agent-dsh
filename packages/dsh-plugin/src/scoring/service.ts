...
export class IdeaScoreService extends Service {
   ...
   *
   * `retrieval_mode` 说的是**召回与证据相似度的口径**：召回走 FTS5 关键词，
   * 证据相似度走字符 trigram → 所以是 `keyword_only`；候选排序同样是字符 trigram。
   *
   * 这里**没有** `rank_mode`：它当初是为了区分"字面/语义"两种排序而加的，
   * 语义那条路已实测否决并删除（§9.1），只剩一种排序，再报一个恒定字段只是噪声。
   */
