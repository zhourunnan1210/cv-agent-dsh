...
The orchestrator (or standalone skill) passes the input contract via the dsh `subagent` tool, choosing a capable model through its model-selection parameter.（dsh 适配：原文为 Claude 的 Agent tool 与 `model: sonnet` 下限，本运行时对应的工具是 `subagent`。） The agent returns the serialised compliance_report. The orchestrator validates against Schema 12 before appending to passport.
