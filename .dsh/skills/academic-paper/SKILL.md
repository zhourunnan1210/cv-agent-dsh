...
> **Routing discipline (v3.9.2):** see `shared/references/intent_clarification_protocol.md` for cross-skill routing rules.（dsh 适配：原句还引用了宿主级 `.claude/CLAUDE.md` 的路由规则，本运行时没有该文件，故移除该引用。） This skill assumes routing has already settled — ambiguous cross-phase materials should have been clarified upstream.
