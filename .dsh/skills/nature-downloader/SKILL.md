...
metadata:
  compatibility: Requires a local Chrome session logged in by the user, Chrome remote debugging permission, Python 3 for configuration, and Node.js 22+ or a bundled Node runtime for download scripts. Uses only user-authorized access. dsh 适配：本仓库把该 skill 装在项目级 skill 根 `<项目根>/.dsh/skills/` 下（原句指向 Claude Code 的 `.claude/skills`）。
---
...
7. The web-access CDP proxy is available or can be started.
   - Typical Claude Code path: `%USERPROFILE%\.dsh\skills\web-access-main\scripts\check-deps.mjs`.
...
$checkDepsCandidates = @(
  "$env:USERPROFILE\.dsh\skills\web-access-main\scripts\check-deps.mjs",
...
```powershell
$env:PYTHONUTF8='1'
python -X utf8 "$env:USERPROFILE\.dsh\skills\sjtu-literature-downloader\scripts\extract_pdf_text.py" `
